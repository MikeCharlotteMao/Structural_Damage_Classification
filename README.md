...
The following files should be downloaded from the processed Z24 Bridge Benchmark dataset:
...
- Damage labels
- 17 structural damage conditions
- Shape: `(1530,)`
...
These files are not included in this repository because they exceed GitHub's file size limit.

### Precomputed Feature File

This repository includes the following precomputed feature file:

```text
X_features_frequency_curvature.npy
```

- Feature vectors extracted using Frequency Domain Decomposition (FDD)
- Each sample contains modal frequency and modal curvature features
- Shape: `(1530, 78)`

This file serves as the input to the machine learning models. It can be regenerated by running `src/FDD.py` using the original dataset.
